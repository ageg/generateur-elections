class Survey:
    def __init__(self, conf):
        ...
        self.welcome_text = ("Bonjour,<br />\n"
                             "<br />\n"
                             f"C'est par honneur, par sens du devoir et par plaisir que je suis votre directeur d'élections pour les élections des {conf.comity} de {conf.type} pour l'{conf.comity_session}.<br />\n"
        ...
        if any(group.unused_posts for group in conf.groups if group.type == "exec"):
            ...
            self.end_text = (
                "Tous les postes seront comblés suite à cette élection régulière. Si vous desirer vous impliquer, nous vous invitons à venir vous renseigner au local de l'AGEG, C1-2045 de la Faculté de Génie.<br />\n"
                "<br />\n"
                "Merci d'avoir participé et bonne journée")
        self.email_invite_subject = f"Élections de {conf.type} - {conf.comity_session}"
        self.email_invite_body = ("<html>\n"
                                  "<head>\n"
                                  "<title></title>\n"
                                  "</head>\n"
                                  "<body>\n"
        ...
        self.email_reminder_body = ("<html>\n"
                                    ...
                                    "<p>Bonjour {FIRSTNAME},<br />\n"
                                    "<br />\n"
                                    f"Vous avez été invité(e) à inscrire votre vote pour {conf.comity} de {conf.type}.<br />\n"
                                    "<br />\n"
                                    f"Nous avons pris en compte que vous n'avez pas encore complété le questionnaire, et nous vous rappelons que celui-ci est toujours disponible si vous souhaitez participer. Vous avez jusqu'à {conf.expiry_text} pour voter.<br />\n"
                                    "<br />\n"
