...
from constants import CONCENTRATION, NOM_USUEL, PHOTO, TEXTE_DESCRIPTIF, POSTE_VISE


def generate_questions(conf, group_conf):
    df = group_conf['df']
    name = f"Comité exécutif de {conf['type']} de l'{group_conf['semester']}"
    description = "Pour cette section, seulement une personne peut-être élue par poste."
    group = Group(name, description)
    postes = conf['postes_promo']
